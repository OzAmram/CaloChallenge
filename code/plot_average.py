...
if __name__ == '__main__':
    ...
    for i in range(nsteps):
        source_file = h5py.File(args.input_file + "generated_step%i.h5" % i, 'r')
        shower, energy = extract_shower_and_energy(source_file, which='input')
        title = "CaloDiffusion: Step %i" %i if i < nsteps -1 else "CaloDiffusion: Final"
        hlf.DrawAverageShower(shower, filename=os.path.join(args.output_dir,
            'average_shower_step{}.{}'.format( str(i), plt_ext)), title=title)
    source_file = h5py.File(args.input_file, 'r')
    shower, energy = extract_shower_and_energy(source_file, which='input', max_evt = 2000)
    title = "Geant4"
    hlf.DrawAverageShower(shower, filename=os.path.join(args.output_dir,
        'average_shower_geant.{}'.format(plt_ext)), title=title)
